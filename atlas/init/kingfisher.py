...
class SraMetadata:
    ...
    def _retry_request(self, description, func):
        ...
        def retrying(i, num_retries=3):
            if i < num_retries - 1:
                logging.warning(
                    "Retrying request (retry {} of {})".format(i + 1, num_retries - 1)
                )
        ...
        for i in range(num_retries):
            try:
                ...
                if not this_res.ok:
                    logging.warning(
                        "Request not OK when {}: {}: {}".format(
                            description, this_res, this_res.text
                        )
                    )
                    logging.warning(
                ...
                retrying(i)
        raise Exception(
            "Failed to {} after {} attempts".format(description, num_retries)
        )
    ...
    def fetch_runs_from_bioprojects(self, bioproject_accessions):
        ...
        query_string = " OR ".join(
            [
                "{}[BioProject]".format(bioproject_accession)
        ...
        if not res.ok:
            raise Exception(
                "HTTP Failure when requesting search from bioproject: {}: {}".format(
                    res, res.text
                )
            )
        ...
        if len(sra_ids) == retmax:
            logging.warning(
                "Unexpectedly found the maximum number of results for this query, possibly some results will be missing"
            )
    ...
    def efetch_metadata_from_ids(self, webenv, accessions, num_ids):
        ...
        if not res.ok:
            raise Exception(
                "HTTP Failure when requesting efetch from IDs: {}: {}".format(
                    res, res.text
                )
            )
        ...
        if root.find("ERROR") is not None:
            logging.error(
                "Error when fetching metadata: {}".format(root.find("ERROR").text)
            )
        ...
        for pkg in root.findall("EXPERIMENT_PACKAGE"):
            ...
            d["experiment_accession"] = try_get(
                lambda: pkg.find("./EXPERIMENT").attrib["accession"]
            )
            ...
            d[STUDY_ACCESSION_KEY] = try_get(
                lambda: pkg.find("./STUDY").attrib["accession"]
            )
            d[BIOPROJECT_ACCESSION_KEY] = try_get(
                lambda: pkg.find(
                    './STUDY/IDENTIFIERS/EXTERNAL_ID[@namespace="BioProject"]'
                ).text
            )
            d["study_alias"] = try_get(lambda: pkg.find("./STUDY").attrib["alias"])
            d["study_centre_project_name"] = try_get(
                lambda: pkg.find("./STUDY/DESCRIPTOR/CENTER_PROJECT_NAME").text
            )
            d["organisation"] = try_get(lambda: pkg.find("./Organization/Name").text)
            d["organisation_department"] = try_get(
                lambda: pkg.find("./Organization/Address/Department").text
            )
            d["organisation_institution"] = try_get(
                lambda: pkg.find("./Organization/Address/Institution").text
            )
            d["organisation_street"] = try_get(
                lambda: pkg.find("./Organization/Address/Street").text
            )
            d["organisation_city"] = try_get(
                lambda: pkg.find("./Organization/Address/City").text
            )
            d["organisation_country"] = try_get(
                lambda: pkg.find("./Organization/Address/Country").text
            )
            first_name = try_get(
                lambda: pkg.find("./Organization/Contact/Name/First").text
            )
            last_name = try_get(
                lambda: pkg.find("./Organization/Contact/Name/Last").text
            )
            d["organisation_contact_name"] = "{} {}".format(first_name, last_name)
            d["organisation_contact_email"] = try_get(
                lambda: pkg.find("./Organization/Contact").attrib["email"]
            )
            # Remove carriage return from sample description e.g. for SRR1263047
            d["sample_description"] = try_get(
                lambda: pkg.find("./SAMPLE/DESCRIPTION").text.replace("\r", "")
            )
            d["sample_alias"] = try_get(lambda: pkg.find("./SAMPLE").attrib["alias"])
            d["sample_accession"] = try_get(
                lambda: pkg.find("./SAMPLE").attrib["accession"]
            )
            d["biosample"] = try_get(
                lambda: pkg.find(
            ...
            d["taxon_name"] = try_get(
                lambda: pkg.find("./SAMPLE/SAMPLE_NAME/SCIENTIFIC_NAME").text
            )
            ...
            if sample_sample_name is not None:
                ...
                d[SAMPLE_NAME_KEY] = d[
                    "library_name"
                ]  # default, maybe there's always a title though?
            d["study_title"] = try_get(
                lambda: pkg.find("./STUDY/DESCRIPTOR/STUDY_TITLE").text
            )
            d["design_description"] = try_get(
                lambda: pkg.find("./EXPERIMENT/DESIGN/DESIGN_DESCRIPTION").text
            )
            d["study_abstract"] = try_get(
                lambda: pkg.find("./STUDY/DESCRIPTOR/STUDY_ABSTRACT").text
            )
            ...
            if study_links_xrefs is not None:
                ...
                for x in study_links_xrefs.findall("STUDY_LINK/URL_LINK"):
                    study_links.append(
                        {"label": x.find("LABEL").text, "url": x.find("URL").text}
                    )
                ...
                if accessions_set is None or accession_here in accessions_set:
                    ...
                    if stats is not None:
                        for i, r in enumerate(stats):
                            d2["read{}_length_average".format(i + 1)] = r.attrib[
                                "average"
                            ]
    ...
    def efetch_sra_from_accessions(self, accessions):
        ...
        if len(all_accessions) > chunk_size:
            logging.info(
                "Querying for {} accessions in {} chunks".format(
                    len(all_accessions), num_chunks
                )
            )
            accession_iter = tqdm(
                iterable_chunks(all_accessions, chunk_size), total=num_chunks
            )
        else:
        ...
        for accessions_plus in accession_iter:
            ...
            if num_chunks == 1:
                logging.info(
                    "Querying NCBI esearch for {} distinct accessions e.g. {}".format(
            ...
            if len(sra_ids) == 0:
                logging.warning(
                    "Unable to find any accessions, from the list: {}".format(
                        accessions
                    )
                )
                return None
            ...
            if num_chunks == 1:
                logging.info(
                    "Querying NCBI efetch for {} distinct IDs e.g. {}".format(
            ...
            if len(metadata) != len(accessions):
                ...
                logging.warning(
                    "Unable to find all accessions. The {} missing ones were: {}".format(
