...
def get_table_from_accessions(accessions, output_file="SRA_run_table.csv" ) -> None:
    ...
    if n_searchable<len(accessions):
        ...
        error_text+= f"I cannot search for {len(not_searchable)} accessions: "\
    ...
    if len(bioproject_accessions)> 0:
        ...
        run_accessions_for_bioprojects = SRA.fetch_runs_from_bioprojects(bioproject_accessions)

        logger.info(f"Found {len(run_accessions_for_bioprojects)} runs for {len(bioproject_accessions)} bioprojects")


        overlap = set(run_accessions) & set(run_accessions_for_bioprojects)

        if len(overlap)>0:
            logger.warning(f"Found {len(overlap)} runs that are in both the bioprojects and the run accessions, downloading only once")
            run_accessions_for_bioprojects = list(set(run_accessions_for_bioprojects) - overlap)
