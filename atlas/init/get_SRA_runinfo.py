    ...
    with open(output_file, "w") as outInfoFile:
        ...
        for identifier in identifier_with_progressbar:
            try:
                ...
                raise Exception("Failed  to get info for " + str(identifier) + ", mess: " + str(err)) from err
...
def parse_arguments_from_terminal():
    ...
    parser.add_argument('-o','--outprefix', type=str, help = "Output prefix for the sra table",
                            default="SRA")
    parser.add_argument('-f','--overwrite', action = "store_true", help = "Overwrite table if it already exists")
    ...
    if os.path.exists(output_file) and not args.overwrite:
        raise Exception("File " + output_file + " already exists, use --overwrite to over write it")
