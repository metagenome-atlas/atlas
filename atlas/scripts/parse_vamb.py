...
clusters['BinID']= clusters_contigs.OriginalName.str.rsplit(separator, n=1, expand=True)[1]
...
for sample, cl in clusters.groupby('Sample'):
    ...
    cl[['Contig','SampleBin']].to_csv(sample_output_path, sep='\t',index=False, header=False )


samples_without_bins= set(snakemake.params.samples).difference(set(clusters.Samples))

if len(samples_without_bins)>0:
    logging.warning("The following samples did't yield bins, I add longest contig to make the pipline continue:\n" + '\n'.join(samples_without_bins)
                    )
