...
if len(fasta_files)>0:
    ...
    Bin_names= gen_names_for_range(N= len(fasta_files), prefix= f"{snakemake.wildcards.sample}_SemiBin_" )
    ...
    pd.concat(mappings,axis=0).to_csv(snakemake.output[0],sep='\t',header=False, index=True)

else:

    logging.warning(f"No bins found in {snakemake.input[0]} add longest contig as bin to make atlas continue.")
