...
rule run_feature_counts:
    ...
    output:
        counts = temp(expand("genomes/expression/gene_counts/{genome}_raw.{ext}",ext=['tsv','tsv.summary']))
    ...
    params:
        ...
        min_overlap = config['feature_counts']['minimum_overlap'],
        frac_overlap = config['feature_counts']['fraction_overlap'],
        frac_overlap_feature = config['feature_counts']['fraction_overlap_feature'],
        min_mapping_qual_score = config['feature_counts']['min_min_mapping_qual_score'],
    ...
    shell:
        """
            ...
            {params.p[max_m_cigar_operations]} \
            --donotsort \
            -T {threads} \
            {input.samples_bam} \
            2> {log}
...
rule parse_feature_counts_output:
    ...
    run:
        ...
        feature_counts_table = pd.read_csv(input, sep='\t', header=1)
        ...
        ORFs = []
        for index, table_row in feature_counts_table.iterrows():
            genome = str(table_row[1]).split(sep='_')[0]
            ORF = "{genome}_{contig_gene}".format(genome=genome,contig_gene=table_row[0])
            ORFs.append(ORF)

        feature_counts_table.insert(loc=0, column='ORF', value=ORFs)
        ...
        pd.DataFrame.to_csv(feature_counts_table, output, sep = '\t', index = False)
